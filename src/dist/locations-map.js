...
function mapboxLocations() {
    ...
    mapgl.on("load", function () {
        ...
        const currentLocationAction = document.querySelector(".current-location-action");
    ...
    mapgl.once("idle", () => {
        const hasAnyLayer = mapgl.getSource("locations-cluster-layer") ||
            mapgl.getSource("locations-nocluster-layer") ||
            mapgl.getSource("locations-highlight-layer");
        if (!hasAnyLayer || !mapgl.isSourceLoaded("locations"))
            return;
    ...
    mapgl.on("mouseleave", "locations-cluster-layer, locations-nocluster-layer, locations-highlight-layer", function () {
        ...
        if (nearestFeature) {
            ...
            listWrapper.animate({
                ...
                $('.office-list-item[data-id="' +
                    featureId +
                    '"] .office-card-wrapper').addClass("active");
            });
        }
    });
    ...
    function countFeatures(item) {
        ...
        if (window.userSearchLongLat !== undefined &&
            ...
            if (items === 1) {
                $(".location-map-notifications .notification-underline").text(items + " Dental Office");
            }
            else {
                $(".location-map-notifications .notification-underline").text(items + " Dental Offices");
            }
            $(".location-map-notifications .notification-continuation").text(" found near your area");
        }
        else {
            if (items === 1) {
        ...
        if (window.userSearchLongLat !== undefined &&
            ...
            notifications.css("display", "flex");
            location_count.text("Available dental clinics:");
            $(".location-map-notifications").css("background-color", "var(--bold-grey)");
            $(".location-map-notifications .notification-underline").text("Closest Office");
            $(".location-map-notifications .notification-continuation").text("is " + window.minDistance.toFixed(1) + " mi. away");
        }
        if (window.userSearchLongLat !== undefined &&
    ...
    //Card button links
    const book_appointment_btn = $(".list-feature .w-dyn-list .office-card-wrapper .office-cta-buttons .btn-primary");
    const directions_btn = $(".list-feature .w-dyn-list .office-card-wrapper .office-cta-buttons .btn-circle");
    ...
    directions_btn.each(function () {
        let directions_slug = "https://www.google.com/maps/search/?api=1&query=Ideal+Dental+" +
            $(this).data("name") +
            "+" +
            $(this).data("address") +
            "++" +
    ...
    function updateVisibleOffices() {
        ...
        const filteredFeatures = locationsGeoJSON.features.filter((feature) => feature.geometry.type === "Point" ||
    ...
    function place_changed_handler(place_search) {
        ...
        const autocompleteSearchBtn = document.querySelector(".form-autocomplete .autocomplete-search-btn");
        ...
        if (input) {
            ...
            input.setAttribute("aria-label", "Your search for: " +
            ...
            input.setAttribute("aria-expanded", "false");
            const edit_address = document.querySelector(".form-autocomplete .edit-address-btn");
            if (edit_address) {
                ...
                edit_address.addEventListener("click", function () {
                    ...
                    input.setAttribute("aria-label", "This address search field contains an autocomplete dropdown list.");
                    input_description_a11.text("This address search field contains an autocomplete dropdown list.");
                    input.setAttribute("aria-expanded", "false");
                    if (autocompleteSearchBtn) {
                        autocompleteSearchBtn.src =
        ...
        mapgl.on("moveend", function (e) {
            ...
            input_description_a11.text("Your search for: " +
                window.userSearchCityRegion +
                " run successfully. Search results are now available.");
            if (!preventReRender) {
                updateVisibleOffices();
            ...
            $(".locations-count").show();
            if (window.userSearchLongLat !== undefined) {
                const offices = document.querySelectorAll(".list-feature .office-list-item .office-card-wrapper");
            ...
            if (window.hasFeatures === false && modalShown === false) {
                ...
                const isMoving = typeof e.target._moving === "boolean"
                    ? e.target._moving
                    : undefined;
                if (isMoving === false) {
                    setTimeout(function () {
                ...
                modal.click(function (e) {
                    ...
                    if ($(e.target).hasClass("show-all-locations-btn") ||
                ...
                const searchArea = turf.circle(userLocation, searchRadiusNotFound, options);
                ...
                if (pointsWithin.features.length > 0) {
                    pointsWithin.features.forEach((point) => {
                        if (point.geometry.type === "Point") {
                            ...
                            const distance = turf.distance(userLocation, pointAsPoint, options);
                ...
                const closest_location_btn = $(".closest-location-wrapper .btn-primary");
                if (nearestLocation) {
        ...
        service.findPlaceFromQuery(request, function (results, status) {
            if (status === google.maps.places.PlacesServiceStatus.OK && results) {
                if (results.length > 0) {
                    ...
                    service.getDetails({ placeId: placeId ? placeId : "" }, function (place, detailStatus) {
    ...
    autocomplete.addListener("place_changed", () => place_changed_handler(autocomplete.getPlace()));
}
